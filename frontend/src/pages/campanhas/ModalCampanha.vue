  ...
  >
    ...
    >
      <q-card-section class="q-pa-none q-px-md">
        ...
        <div class="row">
          Los mensajes siempre se enviarán durante el horario comercial y en días laborables..
        </div>
      </q-card-section>
      <q-card-section class="q-pb-none">
        <div class="row q-gutter-sm">
          ...
          <q-datetime-picker
            ...
            error-message="No puede ser menor que el día actual"
          />
          <q-select
            rounded
            dense
...
export default {
  ...
  methods: {
    ...
    onRejectedFiles (rejectedEntries) {
      this.$q.notify({
        html: true,
        message: `Ops... Ocorrió un error! <br>
        <ul>
          <li>El archivo no debe tener más de 10 MB.</li>
          <li>Priorizar el envío de imágenes o vídeos.</li>
        </ul>`,
        type: 'negative',
        progress: true,
        position: 'top',
    ...
    async handleCampanha () {
      if (this.campanha.message1 === this.campanha.message2 || this.campanha.message1 === this.campanha.message3 || this.campanha.message2 === this.campanha.message3) {
        this.$q.notify({
          type: 'negative',
          message: 'Los mensajes no pueden ser los mismos'
        })
        return
      }
      if (this.campanha.delay < 61) {
        this.$q.notify({
          type: 'negative',
          message: 'El campo de retardo debe ser al menos 61'
        })
        return
      }
      ...
      try {
        ...
        console.error(error)
        this.$notificarErro('Algún problema al crear una campaña', error)
      }
    }
  }
